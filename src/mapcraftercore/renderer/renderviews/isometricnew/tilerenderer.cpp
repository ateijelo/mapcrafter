...
TileTopBlockIterator::TileTopBlockIterator(const TilePos &tile, int block_size, int tile_width)
    ...
    mc::ChunkPos topright_chunk =
        mc::ChunkPos::byRowCol(4 * tile_width * tile.getY(), 2 * tile_width * (tile.getX() + 1));
...
bool TileTopBlockIterator::end() const { return is_end; }

BlockRowIterator::BlockRowIterator(const mc::BlockPos &block) { current = block; }
...
bool BlockRowIterator::end() const { return current.y < 0; }

bool RenderBlock::operator<(const RenderBlock &other) const { return pos < other.pos; }
...
void NewIsometricTileRenderer::renderTile(const TilePos& tile_pos, RGBAImage& tile) {
        ...
        for (old::TileTopBlockIterator it(tile_pos, block_size, tile_width);
                ...
                for (old::BlockRowIterator block(it.current); !block.end(); block.next()) {
                        ...
                        if (current_chunk == nullptr || current_chunk->getPos() !=
current_chunk_pos)
                                // get chunk if not
...
extra_data); else image = images->getBlock(id, data, extra_data);
*/
