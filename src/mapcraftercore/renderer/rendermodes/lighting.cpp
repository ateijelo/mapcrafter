...
uint8_t LightingData::getBlockLight() const { return block_light; }

uint8_t LightingData::getSkyLight() const { return sky_light; }
...
LightingData LightingData::estimate(const mc::Block &block, RenderedBlockImages *block_images,
    ...
    */
    ...
    mc::Block above;
    while (++off.y) {
        above = world->getBlock(block.pos + off, current_chunk, mc::GET_ID | mc::GET_SKY_LIGHT);
...
void LightingRenderMode::draw(RGBAImage &image, const BlockImage &block_image,
    ...
    */
    ...
    // waterlogged blocks: simple but water surface smooth (aka. lighting type
    // smooth_top_simple_rest ?) slabs: smooth transparent, except full water, except ice smooth
